...
export default defineConfig(({ mode }) => {
  ...
  return {
    base: process.env.FIGMA_PUBLIC_URL ? `${process.env.FIGMA_PUBLIC_URL}/` : '/Bageecha/',
    build: {
...
/**
 * Live online counter via Server-Sent Events. Tracks how many
 * browser tabs are currently connected and pushes the count to every
 * client whenever it changes.
 *
 * Dev-only: `apply: 'serve'` gates the plugin to `vite dev` and
 * `vite preview`. Prod builds (`vite build`) skip it entirely so the
 * route doesn't leak into shipped bundles. The actual SSE logic lives
 * in `online-counter.mjs` and is shared with the production server in
 * `server.mjs`.
 */
function figmaOnlineCounter(): Plugin {
  return {
    name: 'figma-online-counter',
    apply: 'serve',
    configureServer(server) {
      server.middlewares.use(onlineCounterMiddleware)
    },
